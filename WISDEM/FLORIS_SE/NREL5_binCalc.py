...
myFloris = floris_assembly_opt_AEP(nTurbines=3, nDirections=1, optimize_yaw=False,
...
def calcProduction(turbine_a, turbine_b, turbine_c, wind):

    a_x = turbine_a["location"]["x"]
    b_x = turbine_b["location"]["x"]
    c_x = turbine_c["location"]["x"]
    a_y = turbine_a["location"]["y"]
    b_y = turbine_b["location"]["y"]
    c_y = turbine_c["location"]["y"]
    ...
    turbineXinit = np.array([a_x, b_x, c_x])
    turbineYinit = np.array([a_y, b_y, c_y])
